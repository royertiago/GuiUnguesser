...
int main ( int argc, char ** argv )
{
	...
	gui.promptMessage ( "Think of an entity. I will try to deduce which entity is." );
	gui.promptMessage ( "After each question, type a number between -1 and 1 to indicate\n"
	                    "if the answer is no or yes or something between." );
	...
	while ( ( move = unguesser.next_move() ) != UnguesserMove::RESTART_GAME ) {
		switch ( move ) {
			...
			case UnguesserMove::GUESS_ANSWER: {
					...
					///       << e->name << "\nIs this right? [y/n] ";
					std::string alceText = "I think the answer is:\n" + e->name + "\nIs this right? [y/n] ";
					double alce = gui.ask ( alceText );
					...
					ans = gui.typeMessage ( "No? Please, type the entity name,so I can adjust my data.\nOr leave it empty, if you want me to try again." );
			...
			case UnguesserMove::GIVE_UP: {
					...
					///   std::getline( std::cin, ans );
					ans = gui.typeMessage ( "I give up. On what are you thinking?" );

					ask_entity ( ans );
					///  std::cout << "Thanks for answering!\n";
					gui.promptMessage ( "Thanks for answering!" );
				}
				break;
			case UnguesserMove::ASK_NEW_QUESTION: {
					...
					alceText += "Please, devise a question to help me differentiate between them.\n Or leave it empty, if you do not want to provide a new question.\n";
					/*
					...
					for ( auto ptr : best ) {
						...
						//    << ptr->name << "?\n";
						alceText = "What is the answer for the entity " + ptr->name + "?";
						double v;
						v = gui.ask ( alceText );
						//std::cin >> v;
						std::cin.ignore();
					...
					gui.promptMessage("Thanks for helping!");
				}
				break;
